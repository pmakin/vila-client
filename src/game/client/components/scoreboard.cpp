...
// ddnet-vila
// https://github.com/TDTW/old-tdtw-0.6-trunk

static constexpr float ScoreboardRounding()
{
	return 10.0f;
}

void CScoreboard::OnRenderEx()
{
	if(Client()->State() != IClient::STATE_ONLINE && Client()->State() != IClient::STATE_DEMOPLAYBACK)
		return;
	...
	const float ScoreboardSmallWidth = 700.0f + 10.0f;
	...
	if(Teams)
	{
		...
		CUIRect RedScoreboard, BlueScoreboard, RedTitle, BlueTitle, TitleBar;
		Scoreboard.HSplitTop(TitleHeight, &TitleBar, &Scoreboard);
		Scoreboard.HSplitTop(Rounding, nullptr, &Scoreboard);
		Scoreboard.VSplitMid(&RedScoreboard, &BlueScoreboard, Rounding);
		TitleBar.VSplitMid(&RedTitle, &BlueTitle, Rounding);

		RedTitle.Draw(ColorRGBA(255, 107, 107, 127).Multiply(1.0f / 255.0f), IGraphics::CORNER_ALL, Rounding);
		BlueTitle.Draw(ColorRGBA(107, 159, 255, 127).Multiply(1.0f / 255.0f), IGraphics::CORNER_ALL, Rounding);

		RenderTitleEx(RedTitle, TEAM_RED, pRedTeamName == nullptr ? Localize("Red team") : pRedTeamName, aTeamSize[TEAM_RED]);
		RenderTitleEx(BlueTitle, TEAM_BLUE, pBlueTeamName == nullptr ? Localize("Blue team") : pBlueTeamName, aTeamSize[TEAM_BLUE]);
		RenderScoreboardEx(RedScoreboard, TEAM_RED, 0, NumPlayers, RenderState);
		Scoreboard.h = RenderScoreboardEx(BlueScoreboard, TEAM_BLUE, 0, NumPlayers, RenderState);
	}
	else
	{
		const char *pTitle;
	...
	if(pGameInfoObj && (pGameInfoObj->m_ScoreLimit || pGameInfoObj->m_TimeLimit || (pGameInfoObj->m_RoundNum && pGameInfoObj->m_RoundCurrent)))
	{
		...
		const float GoalsHeight = Rounding * 2 + LineHeight * LinesNum + (Rounding / 2) * (LinesNum - 1);
		CUIRect Goals = {(Width - ScoreboardSmallWidth) / 2.0f, Height - GoalsHeight, ScoreboardSmallWidth, GoalsHeight};
		RenderGoalsEx(Goals, LineHeight);
	}

	{
		...
		const float SpectatorsWidth = RemainingSpectators <= 16 ? (ScoreboardWidth == ScoreboardSmallWidth ? ScoreboardSmallWidth : 750.0f + 10.0f) : 1500.0f;
		CUIRect Spectators = {(Width - SpectatorsWidth) / 2.0f, Scoreboard.y + Scoreboard.h + Rounding * 1.5f, ScoreboardSmallWidth, Height};

		const char *pTitle = Localize("Spectators");
		CUIRect Title;
		Spectators.HSplitTop(TitleHeight, &Title, &Spectators);
		Spectators.HSplitTop(Rounding, nullptr, &Spectators);
		Title.Draw(ColorRGBA(182, 99, 234, 127).Multiply(1.0f / 255.0f), IGraphics::CORNER_ALL, Rounding);
		RenderTitleEx(Title, TEAM_SPECTATORS, pTitle, RemainingSpectators);

		RenderMultiBoard(Spectators, TEAM_SPECTATORS, RemainingSpectators, RenderState);
	}

	RenderRecordingNotification((Width / 7) * 4 + 20);
}

// TODO: refactor to use features of current client-side codebase, this code was born in 2011
void CScoreboard::RenderGoalsEx(CUIRect Goals, float LineHeight)
{
...
float CScoreboard::RenderScoreboardEx(CUIRect Scoreboard, int Team, int CountStart, int CountEnd, CScoreboardRenderState &State)
{
	const CNetObj_GameInfo *pGameInfoObj = GameClient()->m_Snap.m_pGameInfoObj;
	const CNetObj_GameData *pGameDataObj = GameClient()->m_Snap.m_pGameDataObj;
	const bool TimeScore = GameClient()->m_GameInfo.m_TimeScore;
	...
	const float CountryOffset = PingOffset - CountryLength;
	const float ClanLength = TextRender()->TextWidth(FontSize, "WWWWWWWW");
	const float ClanOffset = CountryOffset - ClanLength - 5.0f;
	const float HeadlineFontsize = 22.0f;

	const float FinalHeight = HeadlineFontsize * 2.0f + (NumPlayers + 0.5f) * (LineHeight + Spacing) - Spacing;
	Scoreboard.h = FinalHeight;

	Scoreboard.Draw(ColorRGBA(0.0f, 0.0f, 0.0f, 0.5f), IGraphics::CORNER_ALL, Rounding);
...
void CScoreboard::RenderTitleEx(CUIRect TitleBar, int Team, const char *pTitle, int NumPlayers)
{
	char aScore[128] = "";
	...
	if(Team != TEAM_SPECTATORS)
	{
		...
		{
			if(GameClient()->m_MapBestTimeMillis)
			{
				str_time(((int64_t)absolute(GameClient()->m_MapBestTimeSeconds)) * 100, TIME_HOURS, aScore, sizeof(aScore));
			}
		}
	...
	{
		{
			char aNumPlayersText[16] = {0};
			str_format(aNumPlayersText, sizeof(aNumPlayersText), "(%d)", NumPlayers);

			const float NumPlayersWidth = TextRender()->TextWidth(TitleFontSize, aNumPlayersText);

			CUIRect NumPlayersLabel;
			if(Team == TEAM_BLUE)
			{
				TitleLabel.VSplitRight(TitleTextWidth, &NumPlayersLabel, &TitleLabel);
				NumPlayersLabel.VSplitRight(10.0f, &NumPlayersLabel, nullptr);
				NumPlayersLabel.VSplitRight(NumPlayersWidth, nullptr, &NumPlayersLabel);
			}
			else
			{
				TitleLabel.VSplitLeft(TitleTextWidth, &TitleLabel, &NumPlayersLabel);
				NumPlayersLabel.VSplitLeft(10.0f, nullptr, &NumPlayersLabel);
				NumPlayersLabel.VSplitRight(NumPlayersWidth, &NumPlayersLabel, nullptr);
			}

			Ui()->DoLabel(&NumPlayersLabel, aNumPlayersText, TitleFontSize - 2.0f, Team == TEAM_BLUE ? TEXTALIGN_MR : TEXTALIGN_ML);
		}
		...
		Ui()->DoLabel(&TitleLabel, pTitle, TitleFontSize, Team == TEAM_BLUE ? TEXTALIGN_MR : TEXTALIGN_ML, Props);
	}

	if(aScore[0] != '\0')
	{
		Ui()->DoLabel(&ScoreLabel, aScore, TitleFontSize, Team == TEAM_BLUE ? TEXTALIGN_ML : TEXTALIGN_MR);
	}
}

float CScoreboard::RenderMultiBoard(CUIRect Scoreboard, int Team, int NumPlayers, CScoreboardRenderState &RenderState)
{
	const float Rounding = ScoreboardRounding();

	if(NumPlayers <= 16)
	{
		Scoreboard.h = RenderScoreboardEx(Scoreboard, Team, 0, NumPlayers, RenderState);
	}
	else if(NumPlayers <= 64)
	{
		int PlayersPerSide;
		...
		CUIRect LeftScoreboard, RightScoreboard;
		Scoreboard.VSplitMid(&LeftScoreboard, &RightScoreboard, Rounding);
		RenderScoreboardEx(LeftScoreboard, Team, 0, PlayersPerSide, RenderState);
		Scoreboard.h = RenderScoreboardEx(RightScoreboard, Team, PlayersPerSide, 2 * PlayersPerSide, RenderState);
	}
	else
	{
		const int NumColumns = 3;
		const int ColumnWidth = (Scoreboard.w - (NumColumns - 1) * Rounding) / NumColumns;
		const int PlayersPerColumn = std::ceil(128.0f / NumColumns);
		CUIRect RemainingScoreboard = Scoreboard;
		for(int i = 0; i < NumColumns; ++i)
		{
			CUIRect Column;
			RemainingScoreboard.VSplitLeft(ColumnWidth, &Column, &RemainingScoreboard);
			RemainingScoreboard.VSplitLeft(Rounding, nullptr, &RemainingScoreboard);
			Scoreboard.h = RenderScoreboardEx(Column, Team, i * PlayersPerColumn, (i + 1) * PlayersPerColumn, RenderState);
		}
	}
